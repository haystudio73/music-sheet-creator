    ...
    <div className="workspace-grid">
      ...
      <main className="main-workspace" id="main-content">
        ...
        {initialLoading || loadingProject ? <div className="workspace-loading" role="status"><LoaderCircle className="spin" size={25}/><p>{initialLoading ? t("Đang kết nối không gian làm việc…") : t("Đang mở dự án…")}</p></div> : !selected ? <>
          ...
          <div className={`upload-zone ${dragging ? 'dragging' : ''} ${busy === 'upload' ? 'uploading' : ''} ${workflowStep === 1 ? 'step-focus' : ''}`} onDragOver={e => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={e => { e.preventDefault(); setDragging(false); if (health)
          ...
          <CollapsibleSection storageKey="audio-analysis" title={<h2>{t("Dò thông số audio")}</h2>} className={`workspace-section ${workflowStep === 2 ? 'step-focus' : ''}`}><AudioAnalysisPanel result={audioAnalysis} busy={busy === 'detect-audio'} disabled={disabled || dirty} analyze={() => void detectAudio()}/></CollapsibleSection>

          {activeJob && <div className={`analysis-progress ${job?.status === 'queued' ? 'queued' : ''} ${workflowStep === 3 ? 'step-focus' : ''}`} role="status"><LoaderCircle className="spin" size={21}/><div><div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}><strong>{job?.status === 'queued' ? t("Đang chờ trong hàng đợi") : t("Đang phân tích bản thu")}</strong>{job?.queue_position ? <span className="auto-analyze-badge" style={{ background: '#d97706' }}>{t("Vị trí #{pos}", { pos: job.queue_position })}</span> : null}</div><p>{t(job?.message || job?.stage || 'Đang chờ worker…')}</p></div><button className="button secondary compact" disabled={Boolean(busy)} onClick={() => void cancel()}>{t("Dừng")}</button></div>}
          {displayedScore && !analysisMode ? <>
            ...
            <CollapsibleSection storageKey="score-sheet" title={<h2>{t("Bản nhạc")}</h2>} className={`workspace-section score-sheet-section ${workflowStep === 4 ? 'step-focus' : ''}`}>
      ...
      <aside className="inspector" aria-label={t("Thiết lập và xuất bản nhạc")}><div className="inspector-heading"><SlidersHorizontal size={17}/><h2>{editingScore ? t("Bản nhạc & xuất file") : t("Thiết lập phân tích")}</h2></div>
        ...
        <CollapsibleSection storageKey="inspector-music-setup" className={`inspector-section ${workflowStep === 3 ? 'step-focus' : ''}`} title={<div className="section-title"><h3>{editingScore ? t("Thiết lập bản nhạc") : t("Thiết lập âm nhạc")}</h3><span className="section-index">01</span></div>}>
